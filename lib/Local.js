...
function Local(){
  ...
  this.addArgs = function(options){
    for(var key in options){
      ...
      case 'localProxyHost':
        if(value)
          this.localProxyHost = value;
        break;

      case 'localProxyPort':
        if(value)
          this.localProxyPort = value;
        break;

      case 'localProxyUser':
        if(value)
          this.localProxyUser = value;
        break;

      case 'localProxyPass':
        if(value)
          this.localProxyPass = value;
        break;
  ...
  this.getBinaryArgs = function(){
    ...
    if(this.localProxyHost){
      args.push('--local-proxy-host');
      args.push(this.localProxyHost);
    }
    if(this.localProxyPort){
      args.push('--local-proxy-port');
      args.push(this.localProxyPort);
    }
    if(this.localProxyUser){
      args.push('--local-proxy-user');
      args.push(this.localProxyUser);
    }
    if(this.localProxyPass){
      args.push('--local-proxy-pass');
      args.push(this.localProxyPass);
    }
